...
const dataEdit = (id) => {
  axios.get(route(ruta + '.asignar', id))
    .then(res => {
      var datos = res.data
      rutas.value.options = datos.rutas
      repartidores.value.options = datos.repartidores
  ...
  axios.get(route(ruta + '.show', id))
    .then(res => {
      ...
      form.ruta_id = datos.ruta_id
      form.repartidor_id = datos.repartidor_id
        ...
        <form @submit.prevent="submit">
          <div class="px-2 grid grid-cols-6 gap-4 md:gap-3 2xl:gap-6 mb-2">
            ...
            <div class="col-span-6 shadow-default xl:col-span-3">
              <InputLabel for="ruta" value="Ruta" class="block text-base font-medium leading-6 text-gray-900" />
              <Multiselect id="ruta" v-model="form.ruta_id" v-bind="rutas">
              </Multiselect>
              <InputError class="mt-1 text-xs" :message="form.errors.ruta_id" />
            </div>
            <div class="col-span-6 shadow-default xl:col-span-3">
              <InputLabel for="repartidor" value="Repartidor"
                class="block text-base font-medium leading-6 text-gray-900" />
              <Multiselect id="repartidor" v-model="form.repartidor_id" v-bind="repartidores">
              </Multiselect>
            ...
            <div class="col-span-6 shadow-default xl:col-span-3">
              <InputLabel for="estado" value="Estado" class="block text-base font-medium leading-6 text-gray-900" />
              <Multiselect id="estado" v-model="form.estado" v-bind="estados">
              </Multiselect>
