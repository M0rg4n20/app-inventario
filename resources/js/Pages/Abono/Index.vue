...
import CrearModal from '@/Pages/Abono/Partials/CrearModal.vue';
...
onMounted(() => {
  console.log(usePage().props.lista_abonos)
  datos.value = usePage().props.lista_abonos;
  tabla()
            ...
            <div class="overflow-visible">
              ...
              <table id="tabla1" class="pt-3 w-full text-sm text-left text-gray-600 dark:text-gray-400">
                ...
                <tbody>
                  ...
                  <tr :key="id"
                    v-for="{ id, codigo, total, monto_efectivo, cliente, monto_tarjeta, saldo, usuario, created_at }, index in datos"
                    ...
                    <td scope="row"
                      class="px-3 py-1 border text-end border-gray-300 dark:border-gray-700 text-gray-900 whitespace-nowrap dark:text-white">
                      {{ (monto_efectivo + monto_tarjeta).toFixed(2) }}
                    </td>
                    <td scope="row"
                      class="px-3 py-1 border text-end border-gray-300 dark:border-gray-700 text-gray-900 whitespace-nowrap dark:text-white">
                      {{ (monto_efectivo).toFixed(2) }}
                    </td>
                    <td scope="row"
                      class="px-3 py-1 border text-end border-gray-300 dark:border-gray-700 text-gray-900 whitespace-nowrap dark:text-white">
                      {{ (monto_tarjeta).toFixed(2) }}
                    </td>
                    <td scope="row"
                      class="px-3 py-1 border text-end border-gray-300 dark:border-gray-700 text-gray-900 whitespace-nowrap dark:text-white">
                      {{ (saldo).toFixed(2) }}
                    ...
                    <td scope="row" class="px-3 py-1 border  border-gray-300 dark:border-gray-700 text-gray-900 text-center whitespace-nowrap dark:text-white">
                      <span class="inline-block rounded bg-sky-300 px-2 py-1 text-base font-semibold text-white mr-1 mb-1 hover:bg-sky-400">
                        <a :href="route('abonos.generar_ticket', codigo)" target="_blank"><i class="fas fa-print"></i></a>
                      </span>
                    </td>
