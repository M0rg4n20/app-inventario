...
const sumaTotalPago = () => {
  form.suma_pago = parseFloat(form.monto_efectivo) + parseFloat(form.monto_tarjeta);
...
const calcularCambio = () => {
  let cambio = parseFloat(form.monto_efectivo) - parseFloat(form.total);
  form.cambio = cambio.toFixed(2);
}
...
const setFormaEntrega = (e) => {
  if (e.target.checked) {
    ...
    form.metodo_pago_id="1";
  } else {
    form.forma_entrega = "DOMICILIO";
    form.metodo_pago_id="0";
  }
}
...
const submit = () => {
  ...
  if ((form.metodo_pago_id === "0") &&  (form.forma_entrega != "DOMICILIO") ) {
    alerta('Debe indicar el método de pago.', 'warning');
    return false;
  }
  ...
  if (form.metodo_pago_id == 3) {
    if (form.monto_tarjeta == 0 || form.monto_tarjeta == '') {
      alerta('Debe ingresar el monto en tarjeta', 'warning');
      return false;
    }
    ...
    if (form.metodo_pago_id == 0 || form.metodo_pago_id == '') {
      alerta('Debe ingresar el método de pago', 'warning');
      return false;
    }

    if (parseFloat(form.saldo)) {
      alerta('El saldo a favor del cliente no aplica para pagos con tarjeta', 'warning');
      return false;
    }
  }

  if (parseFloat(form.saldo) > 0 && form.forma_entrega === "INMEDIATA" && form.tipo_pago !='ABONO') {
          ...
          <form @submit.prevent="submit">
            <div class="px-2 grid grid-cols-12 gap-2 lg:gap-2 mt-0 mb-2">
              ...
              <div class="grid grid-cols-12 gap-2  mt-0 col-span-full">
                <div class="col-span-8  lg:col-span-9">
                  <InputLabel for="cliente_id" :value="'Cliente -- ' + tipo_cliente"
                    class="block text-base font-medium leading-6 text-gray-900" />
                  <Multiselect id="cliente_id" v-model="form.cliente_id" v-bind="clientes" @clear="limpiarFormulario"
              ...
              <div class="grid grid-cols-12  mt-0 col-span-full text-start">
                ...
                <!-- v-if="form.tipo_pago == 'ABONO'" -->
                <div class="grid mt-3 grid-cols-12 gap-3 col-span-full  lg:col-span-12 text-center">

                  <!-- Cambio -->
                  <div class="col-span-6 lg:col-span-8 flex pt-3 mt-3 justify-end" v-if="form.cambio > 0">
                  </div>
                  <div class="col-span-6 lg:col-span-4" v-if="form.cambio > 0">
                    <InputLabel for="Cambio" value="Cambio" class="block text-xs font-normal leading-6 text-gray-900" />
              ...
              <div v-if="form.forma_entrega == 'DOMICILIO'" class="grid grid-cols-12 gap-2 col-span-full  py-2 my-2 border-t-2 border-b-2 lg:gap-2">                
                <div class="col-span-6">
