...
const lista_colonias = ref('');



const addCliente = () => {
    form.clearErrors()
    axios.get(route(ruta + '.colonias'))
    .then(res => {
        var datos = res.data.data
        lista_colonias.value = datos
...
//check colonias
const check = (valor, checked) => {
    if (checked) {
        form.colonias.push(valor);
    } else {
        form.colonias.splice(form.colonias.indexOf(valor), 1);
    }
};
            ...
            <div class="p-2">
                ...
                <form @submit.prevent="submit">
                    <div class="px-2 grid grid-cols-6 gap-4 md:gap-3 2xl:gap-6 mb-2">
                        <div :key="index" class="col-span-12  xl:col-span-3 m-0.5" v-for="col, index in lista_colonias">
                            <CheckboxName
                            :key="index"
                            @update:checked="check(col,$event)"
                            :fieldId="'index'"
                            :label="col" >
                            </CheckboxName>

                        </div>
                        <div class="col-span-12  xl:col-span-6 m-0.5">

                            <InputError class="mt-1 text-xs" :message="form.errors.colonias" />
                        </div>

                       <div class="col-span-6 shadow-default xl:col-span-6">
                            <InputLabel for="nombre" value="Nombre"
                                class="block text-base font-medium leading-6 text-gray-900" />
                            <TextInput id="nombre" type="text" v-model="form.nombre" @keyup="mayuscula($event)" autocomplete="nombre"
